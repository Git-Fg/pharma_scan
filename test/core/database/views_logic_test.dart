...
import '../../test_utils.dart'
    show
        findHospitalOnlyMedication,
        findMedicationWithCondition,
        findNarcoticMedication,
        getRealCis,
        loadRealBdpmData,
...
void main() {
  ...
  group('SQL Views Logic - Computed Flags', () {
    ...
    setUp(() async {
      ...
      // Hybrid approach: Load real BDPM data as base
      // Tests will try to find medications with specific conditions (narcotic, hospital, etc.)
      // in real data. If not found, SeedBuilder is used to add controlled test cases.
      // This ensures tests work with real data when available, but remain reliable
      // when specific conditions don't exist in the dataset.
    ...
    test('is_narcotic flag activates for "Stupéfiant" and "Liste I"', () async {
      // Try to find narcotic medications in real data
      final realNarcoticCis = await findNarcoticMedication(database);
      final realList1Cis = await findMedicationWithCondition(
        database,
        'Liste I',
      );
      final realNormalCis = await getRealCis(database);

      // If specific conditions not found in real data, use SeedBuilder for controlled test
      if (realNarcoticCis == null || realList1Cis == null) {
        await SeedBuilder()
        ...
        expect(
          ...
          isFalse,
          reason:
              'Liste I should NOT activate is_narcotic (only Liste II does)',
        );
        expect(
          normal.isNarcotic,
          isFalse,
        ...
        final narcotic = await (database.select(
          database.medicamentSummary,
        )..where((tbl) => tbl.cisCode.equals(realNarcoticCis))).getSingle();
        final list1 = await (database.select(
          database.medicamentSummary,
        )..where((tbl) => tbl.cisCode.equals(realList1Cis))).getSingle();
        final normal = await (database.select(
          database.medicamentSummary,
        )..where((tbl) => tbl.cisCode.equals(realNormalCis))).getSingle();

        expect(
          narcotic.isNarcotic,
          isTrue,
          reason: 'Stupéfiant from real data should activate is_narcotic',
        );
        expect(
          list1.isNarcotic,
          isFalse,
          reason: 'Liste I from real data should NOT activate is_narcotic',
        );
        expect(
          normal.isNarcotic,
          isFalse,
          reason:
              'Normal medication from real data should NOT activate is_narcotic',
        );
      }
    });

    test('is_hospital flag activates for hospital-only conditions', () async {
      // Try to find hospital-only medication in real data
      final realHospitalCis = await findHospitalOnlyMedication(database);
      final realNormalCis = await getRealCis(database);

      // If not found in real data, use SeedBuilder for controlled test
      if (realHospitalCis == null) {
        await SeedBuilder()
        ...
        final hospital = await (database.select(
          database.medicamentSummary,
        )..where((tbl) => tbl.cisCode.equals(realHospitalCis))).getSingle();
        final normal = await (database.select(
          database.medicamentSummary,
        )..where((tbl) => tbl.cisCode.equals(realNormalCis))).getSingle();
        ...
        expect(
          ...
          isTrue,
          reason:
              'Hospital condition from real data should activate is_hospital',
        );
        expect(
          normal.isHospitalOnly,
          isFalse,
          reason:
              'Normal medication from real data should NOT activate is_hospital',
        );
      }
    });

    test('is_list2 flag activates for "Liste II"', () async {
      // Try to find Liste II and Liste I medications in real data
      final realList2Cis = await findMedicationWithCondition(
        database,
        'Liste II',
      );
      final realList1Cis = await findMedicationWithCondition(
      ...
      // If not found in real data, use SeedBuilder for controlled test
      if (realList2Cis == null || realList1Cis == null) {
        await SeedBuilder()
        ...
        final list2 = await (database.select(
          database.medicamentSummary,
        )..where((tbl) => tbl.cisCode.equals(realList2Cis))).getSingle();
        final list1 = await (database.select(
          database.medicamentSummary,
        )..where((tbl) => tbl.cisCode.equals(realList1Cis))).getSingle();
        ...
        expect(
          list2.isList2,
          isTrue,
          reason: 'Liste II from real data should activate is_list2',
        );
        expect(
          list1.isList2,
          isFalse,
          reason: 'Liste I from real data should NOT activate is_list2',
        );
      }
    });
