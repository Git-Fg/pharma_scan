...
class DataInitializationService {
    ...
    try {
      ...
      if (hasVersion && !forceRefresh) {
        try {
          ...
          integrityOk = false;
          _logger.warning(
            '[DataInit] Integrity check failed, requiring reset.',
          );
        }
      }
      ...
      if (!needsInitialization) {
        ...
        _emit(.ready, Strings.initializationReady);
        return;
      }

      // 3. Hydrate from Bundled Asset
      _logger.info('[DataInit] Initializing database from bundled asset...');
      _emit(.downloading, 'Préparation de la base de données...');
      ...
      _logger.error('[DataInit] Error during initialization', e, stackTrace);
      _emit(.error, Strings.initializationError);
      rethrow;
    }
  }
  ...
  Future<bool> updateDatabase({bool force = false}) async {
    try {
      ...
      if (response.statusCode != 200 || response.data == null) {
        _logger.warning('[DataInit] GitHub API error: ${response.statusCode}');
        return false;
      }
      ...
      if (!force) {
        ...
        if (currentTag == latestTag) {
          _logger.info('[DataInit] Database is up to date ($currentTag)');
          return false;
        }
      ...
      _emit(.downloading, 'Mise à jour de la base...');
      ...
      _logger.error('[DataInit] Error during database update', e, stackTrace);
      _emit(.error, Strings.initializationError);
      return false;
    }
  }
  ...
  Future<VersionCheckResult?> checkVersionStatus({
    ...
    try {
      ...
      try {
        if (localDate != null) {
          final cleanLocal = localDate.replaceAll('db-', '');
          localDateTime = DateTime.tryParse(cleanLocal);
        }

        final cleanRemote = latestTag.replaceAll('db-', '');
        remoteDateTime = DateTime.tryParse(cleanRemote);

        if (remoteDateTime != null) {
          if (localDateTime == null) {
            updateAvailable = true;
          } else {
            updateAvailable = remoteDateTime.isAfter(localDateTime);
          }
        } else {
        ...
        _logger.warning('[DataInit] Version comparison failed: $e');
        updateAvailable = localDate != latestTag;
      }
