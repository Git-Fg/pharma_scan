DOCUMENTATION A TITRE INDICATIVE, VOUE A CHANGER EN FONCTION DES INVESTIGATIONS FUTURES
...
6. Si plusieurs candidats de type 0 existent, appliquer un scoring : +10 pour tout type 0, +20 supplémentaire pour les golden (sort = 1), -50 si le libellé commence par la molécule normalisée (évite de choisir un auto-générique “FLUCONAZOLE PFIZER” devant “TRIFLUCAN”). En cas d’égalité, choisir le libellé le plus court.
...
* **Logique SA vs FT :**
  ...
  * Prendre le second numéro : si FT, utiliser celui ci. Sinon FA
  ...
* **Agrégation Cluster :** Utiliser les noms de substances normalisés comme clé principale de signature (sels effacés) et n’utiliser les codes substances (col 3) qu’en secours ; tokens triés pour stabilité. Tous les CIS d'un même Groupe doivent aboutir à la même signature, même en présence de variantes sel/base.
